function [maxstate logprob]=HMMviterbiGeneral(phghm,ph1,pvgh)
...
import brml.*
T=size(pvgh,2); H=size(phghm,1);
mu(:,T)=ones(H,1);
for t=T:-1:2
	tmp = repmat(pvgh(:,t).*mu(:,t),1,H).*phghm;
	mu(:,t-1)= condp(max(tmp)'); % normalise to avoid underflow
end
% backtrack
[val hs(1)]=max(ph1.*pvgh(:,1).*mu(:,1));
for t=2:T
	tmp = pvgh(:,t).*phghm(:,hs(t-1));
	[val hs(t)]=max(tmp.*mu(:,t));
end
maxstate=hs;
logprob=log(ph1(hs(1)))+log(pvgh(hs(1,t)));
for t=2:T
	logprob=logprob+log(phghm(hs(t),hs(t-1)))+log(pvgh(hs(t),t));
end