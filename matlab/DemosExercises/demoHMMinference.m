...
fprintf(1,'Some different (mathematically equivalent) ways to perform inference in a HMM:\n\n')
import brml.*
...
pot{vt(1)}=array([vt(1) ht(1)], pvgh);
for t=2:T
    pot{vt(t)}=array([vt(t) ht(t)],pvgh);
    pot{ht(t)}=array([ht(t) ht(t-1)],phghm);
end
...
FGloglik = log(sum(tmpmess.table)); 
for t=1:T
    post(:,t)=condp(marg{t}.table);
end
